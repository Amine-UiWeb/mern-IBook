...
const toggleActive = (target, parent, child) => {
  target.closest(parent).querySelectorAll(child).forEach(el => el.classList.remove('active'))
...
const SearchPaginList = ({ authorKey, totalWorks }) => {
  ...
  // todo (lastly): disable all sort and pagination buttons on fetchLoading
  ...
  const [sortBy, setSortedBy] = useState(null)
  ...
  const worksWrapper = (
    <>
      ...
      {fetchError && 
        <NoSectionData message="Network propblem. couldn't fetch the requested data" />
      }
    </>
  )
